...
class Kuramanime :
    ...
    /**
     * Interceptor to gracefully fallback to external Scraper API (e.g. https://s1allsolver.up.railway.app or https://rbot.duar.eu.cc)
    ...
    private class ScraperFallbackInterceptor(
        ...
        override fun intercept(chain: Interceptor.Chain): Response {
            ...
            val customSolverApi = preferences.getString(PREF_SCRAPER_URL_KEY, DEFAULT_SCRAPER_URL)!!.trimEnd('/')

            val host = request.url.host
            if (!useScraper || host.contains("railway.app") || host.contains("duar.eu.cc") || request.url.encodedPath.contains("cf-clearance-scraper")) {
            ...
            try {
                ...
                if (res.code in listOf(403, 503, 520, 521, 522, 524) || res.header("cf-mitigated") != null) {
                    needFallback = true
                    res.close()
                } else {
                    val peek = runCatching { res.peekBody(1024).string() }.getOrDefault("")
                    if (peek.contains("Just a moment...", ignoreCase = true) ||
            ...
            if (needFallback && request.method == "GET") {
                ...
                for (apiUrl in solverUrls) {
                    try {
                        ...
                        val jsonPayload = if (apiUrl.contains("railway.app")) {
                            """{"url":"${request.url}","timeout":45}"""
                        } else {
                            """{"url":"${request.url}","mode":"source"}"""
                        }
                        ...
                        if (scraperResponse.isSuccessful) {
                            ...
                            if (htmlContent.isNotBlank() && !htmlContent.contains("Attention Required! | Cloudflare")) {
    ...
    companion object {
        ...
        private const val DEFAULT_SCRAPER_URL = "https://s1allsolver.up.railway.app"
